...
// ---------------------------------------------------------------
// 3. TECHNOLOGY DATA (formerly WEAPONS)
// ---------------------------------------------------------------
const technologyData = (() => {
  let data = null;
  return async function getTechnology() {
    if (data) return data;
    const script = document.getElementById("technology-data");
    if (script) {
      data = window.technologyData || [];
      return data;
    }
    try {
      const response = await fetch("data/technology.js");
      const text = await response.text();
      const fn = new Function(text + "\n window._technologyData = technologyData;");
      fn();
      data = window._technologyData || [];
      delete window._technologyData;
      return data;
    } catch (e) {
      console.error("Failed to load technology data:", e);
      return [];
    }
  };
})();
...
/**
 * Get a technology item by its unique id.
 * @param {number} id
 * @returns {Promise<Object|null>}
 */
async function getTechnologyById(id) {
  const all = await technologyData();
