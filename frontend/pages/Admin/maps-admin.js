...
async function loadRecords(page = 1) {
  ...
  try {
    ...
    container.innerHTML = `<p style="color:var(--text-muted)">Maps unavailable.</p>`;
  }
}

function renderList(container, { data, total, page, pages }) {
  if (!data.length) { container.innerHTML = `<p style="color:var(--text-muted)">No maps yet. Create one above.</p>`; return; }
  container.innerHTML = `
    <p style="font-size:var(--text-xs);color:var(--text-muted);margin-bottom:var(--space-4);">${total} maps · page ${page} of ${pages}</p>
...
async function showPreview() {
  ...
  try {
    ...
    content.innerHTML = `
      ...
      <p style="color:var(--text-muted);margin-bottom:var(--space-2);">${[rendered.theater, rendered.year].filter(Boolean).join(" · ")}</p>
