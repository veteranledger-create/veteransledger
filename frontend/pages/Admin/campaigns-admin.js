...
async function loadCampaigns(page = 1) {
  ...
  try {
    ...
    container.innerHTML = `<p style="color:var(--text-muted)">Campaigns unavailable.</p>`;
  }
}

function renderList(container, { data, total, page, pages }) {
  if (!data.length) {
    container.innerHTML = `<p style="color:var(--text-muted)">No campaigns yet. Create one above.</p>`;
    return;
  }
  container.innerHTML = `
    <p style="font-size:var(--text-xs);color:var(--text-muted);margin-bottom:var(--space-4);">${total} campaigns · page ${page} of ${pages}</p>
    <table style="width:100%;border-collapse:collapse;font-size:var(--text-sm);">
      <thead>
        <tr style="border-bottom:1px solid var(--border-dim);color:var(--text-muted);text-align:left;">
          ...
          <th style="padding:var(--space-3) var(--space-4);">Theater</th>
          <th style="padding:var(--space-3) var(--space-4);">Start</th>
      ...
      <tbody>
        ${data.map((r) => {
          ...
          <tr style="border-bottom:1px solid var(--border-dim);">
            ...
            <td style="padding:var(--space-3) var(--space-4);"><span class="badge">${escHtml(theater)}</span></td>
            <td style="padding:var(--space-3) var(--space-4);color:var(--text-muted);">${escHtml(start)}</td>
...
async function showPreview() {
  ...
  try {
    ...
    content.innerHTML = `
      ...
      <p style="color:var(--text-muted);margin-bottom:var(--space-1);">${escHtml(rendered.theater || "")}${dates.start ? " · " + dates.start : ""}</p>
