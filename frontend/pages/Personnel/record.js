...
function render(root, p) {
  ...
  root.innerHTML = `
    <nav class="record-breadcrumb" aria-label="Breadcrumb">
      ...
      ${branchLabel ? `<span>${branchLabel}</span><span class="icon-svg icon-svg--angle-right icon-svg--sm record-breadcrumb__sep" aria-hidden="true"></span>` : ""}
      <span>${p.name || ""}</span>
    </nav>
...
function renderError(root, msg) {
  root.innerHTML = `
    <nav class="record-breadcrumb"><a href="/personnel">← ${t("breadcrumb.backToPersonnel", null, "Back to Personnel")}</a></nav>
